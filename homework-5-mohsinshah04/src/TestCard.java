...
import static edu.uwm.cs351.Card.Suit.*;
...
public class TestCard extends LockedTestCase {
	...
	private void testGroup(Card.Group g) {
		...
		g.add(c[4]);
		g.add(c[7]);
		assertEquals(Ti(669468710),asInt(() -> c[7].getPrev()));
	...
	private void testSort(Card.Group g) {
		...
		// acesHigh has A high, then K, Q, J, 10, 9 ... 2
	...
	public void test27() {
		g.add(c[3]);
		g.add(c[4]);
		assertNull(c[3].getPrev());
		assertNull(c[4].getNext());
		assertSame(c[3],c[4].getPrev());
	...
	public void test31() {
		...
		assertSame(c[2],c[4].getPrev());
		assertSame(c[6],c[4].getNext());
		assertSame(c[4],c[6].getPrev());
	...
	public void test69() {
		...
		assertNull(c[6].getPrev());
		
		g.remove(c[5]);
		assertSame(c[3],c[7].getPrev());
	...
	public void test83() {
		...
		assertSame(deuce,ace.getPrev());
		assertSame(g,ace.getGroup());
		assertSame(ace,deuce.getNext());
		assertNull(deuce.getPrev());
		assertSame(g,deuce.getGroup());
	...
	public void test85() {
		...
		assertNull(ace.getPrev());
		assertEquals(ace,deuce.getPrev());
		assertEquals(deuce,king.getPrev());
	...
	public void test86() {
		...
		assertSame(null,kingHeart.getPrev());
		assertSame(aceSpade,kingHeart.getNext());
		assertSame(kingHeart,aceSpade.getPrev());
		assertSame(kingTrump,aceSpade.getNext());
		assertSame(aceSpade,kingTrump.getPrev());
		assertSame(aceTrump,kingTrump.getNext());
		assertSame(kingTrump,aceTrump.getPrev());
